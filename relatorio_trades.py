...
def generate_trades_report(config: dict):
    ...
    for file in trade_files:
        ...
        # PnL percentual
        # --------------------------------------------------
        df['pnl_percent'] = df['return'] * 100  # return já é percentual em decimal

        pdf_path = os.path.join(
            output_folder,
            f"{symbol}_trades_detailed.pdf"
        )
        ...
        elements.append(
            Paragraph(
                f"<b>{symbol} — Trade Level Report</b>",
                styles["Title"]
            )
        )
        ...
        for year in range(start_year, end_year + 1):
            ...
            elements.append(
                Paragraph(f"<b>Year: {year}</b>", styles["Heading1"])
            )
